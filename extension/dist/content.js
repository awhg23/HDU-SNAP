(() => {
  // src/content/automation-policy.js
  function nextActionAfterDecision(snapshot) {
  ...
  // src/content/review-state.js
  function activateReviewMode(state2, { phase, recordOpened }) {
    state2.reviewEnabled = true;
    state2.reviewPhase = phase;
    state2.reviewRecordOpened = recordOpened;
    state2.reviewQueue = [];
    state2.reviewCollected = /* @__PURE__ */ new Set();
    state2.reviewResults = [];
    state2.reviewResultsSent = false;
    state2.reviewVisited = /* @__PURE__ */ new Set();
    state2.reviewNavigationMode = "card";
  }

  // src/content/review.js
  ...
  function pageShowsWrongStatus(text) {
  ...
  function classTrail(element) {
    return elementTrail(element, 4).map((node) => String(node.className || "")).join(" ").toLowerCase();
  }
  ...
  function findQuestionElement() {
    ...
    const candidates = queryVisible("#app *").map((element) => ({ element, text: visibleText(element) })).filter(({ text }) => text && text.length >= 2 && text.length <= 60).filter(({ text }) => !/^(开始|提交|确认|返回|继续|题卡|自动下一题|下一项|下一题)$/.test(text)).filter(({ text }) => !parseOptionLine(text)).filter(({ text }) => hasChinese(text) || /^[a-zA-Z][a-zA-Z\s'-]{1,40}$/.test(text)).sort((left, right) => scoreQuestionText(right.text) - scoreQuestionText(left.text));
  ...
  function findQuestionCardButtons() {
    const rawCandidates = queryVisible("button, [role='button'], .van-button, .btn, a, span, div").filter((element) => /题卡/.test(visibleText(element))).map((element) => toClickableCandidate(element)).filter(Boolean);
    ...
    })).filter(({ text, rect }) => /题卡/.test(text) && rect.width > 0 && rect.height > 0).sort((left, right) => {
  ...
  function findLatestHistoryRecord() {
    const candidates = queryVisible("a, button, [role='button'], .van-cell, .van-card, li, div").map((element) => {
      const text = visibleText(element);
      const rect = element.getBoundingClientRect();
      return { element, text, rect };
    }).filter(({ text, rect }) => text && text.length <= 120 && rect.height >= 24).filter(({ text }) => !textLooksLikeButton(text)).filter(({ text }) => !/^(历史记录|做题记录|答题记录|题卡|返回|关闭|确认)$/.test(text)).sort((left, right) => left.rect.top - right.rect.top);
  ...
  function findReviewNextButton() {
    const candidates = queryVisible("button, [role='button'], .van-button, .btn, a, span, div").map((element) => {
      const text = visibleText(element);
      const rect = element.getBoundingClientRect();
      const cls = classTrail(element);
      return { element, text, rect, cls };
    }).filter(({ rect }) => rect.width >= 24 && rect.height >= 24).filter(({ text }) => !/题卡/.test(text)).filter(({ rect }) => rect.bottom >= window.innerHeight - 120).sort((left, right) => right.rect.right - left.rect.right);
  ...
  async function runReviewStep() {
    ...
    if (state.reviewNavigationMode === "card") {
      if (currentItemId && state.reviewQueue.includes(currentItemId) && !state.reviewCollected.has(currentItemId)) {
        const reviewResult2 = buildReviewResult();
        if (reviewResult2) {
          state.reviewResults.push(reviewResult2);
          state.reviewCollected.add(currentItemId);
          debugLog("collected review result", reviewResult2);
        }
      }
