...
- Xcode 已安装，Electron 开发进程和 Apple Silicon sidecar 构建链路均已验证。
...
- 2.1.0 未签名 DMG 已于 2026-08-17 成功生成；构建后校验和只读挂载检查均确认镜像内补丁与仓库基线逐字节一致。后续 2.3.0 已完成覆盖安装和真实答题验收，此项不再待验收。
...
- 2.2.0 桌面 UI 已统一为奶油纸、陶土、芥末黄和深橄榄的暖色视觉，不得恢复蓝白商务风。设计规则保存在 `design-system/hdu-snap/MASTER.md`；图标使用内嵌 SVG，首页与首次引导插画位于 `desktop/src/renderer/assets/study-companion.png`，所有视觉资源必须随 App 本地打包，不得引入网络字体或远程图片。
- 2.2.0 结果页提供“记录错题”手动单题入口：用户自行翻到一条错题后点击按钮，站点适配器只扫描当前题的题目、A–D 选项和页面标示的正确答案，主进程校验后写入共享补丁。该能力不得自动翻页、批量采集、写调试记录或恢复调试模式；自动测试和渲染冒烟已通过，尚待真实结果页人工验收。
- 设置页的手动补丁表单草稿在当前 App 进程内跨页面、跨设置标签保留，保存成功后清空；草稿不得写入批次记录或诊断状态。记录、设置和诊断页使用紧凑布局，不显示营销式大标题文案。补丁文件保持原顺序，补丁库仅在 UI 中倒序展示，使最新保存或导入的规则位于最上方。
- 创建答题任务后，主进程必须在学习首页导航完成后返回最新公共状态，renderer 再切换到学习页，以保证待就绪批次和“开始答题”按钮立即出现。原生网页视图新建时保持隐藏，可见性只由当前 renderer 页面显式驱动，不得使用可能与主进程失步的 renderer 可见性缓存；否则网页会覆盖首页或设置页。
- 首页每次启动默认 100 题，快捷题量固定为 90、95、100；进入学习站点必须实际导航到设置的学习首页，而不是继续停留在上一轮题目页。
- 2.3.0 起删除共享 Solver 的本地向量模型层。决策顺序固定为“补丁规则 → 词典 → DeepSeek → 确定性兜底”；配置、自检、依赖和安装包均不再包含 Torch、Sentence Transformers、M3E 模型或向量阈值。为兼容第一阶段旧插件，`/health` 暂时保留旧向量字段名，但值固定为 `removed`，不得据此恢复模型加载。
...
- 用户已确认 2.2.0 源码版真实答题、调试模式退场、顶部运行栏、居中答题画布和右侧答题详情均无问题。2.3.0 未签名 DMG 已于 2026-08-17 成功生成，大小为 139,453,489 字节；只读挂载、arm64 架构、内置补丁、无向量依赖和 sidecar 初始化均已校验，并已完成覆盖安装和真实答题验收。`v2.3.0` 已作为正式版发布到 GitHub Release；旧插件、CLI、脚本和本地 API 的退场作为后续独立变更执行。
...
1. 首次启动执行系统、核心、词典与补丁资源、数据目录、网页组件和目标站点网络自检。
...
9. 运行中在移动画布右侧显示仅当前进程保留的逐题详情；提交确认后只保存批次摘要。Mac App 不自动复盘、不持久化逐题内容；用户可以在结果页手动翻到当前错题并点击一次“记录错题”，也可通过设置页手动添加或导入旧版 JSON/JSONC 补丁。
...
4. 后端按“补丁规则 → 词典 → DeepSeek → 确定性兜底”的顺序选择答案。
...
- 未配置 LLM 或 LLM 重试耗尽后固定选择 A 作为可审计的确定性兜底，决策方法必须标记为“确定性兜底”，不得伪装成“大模型决策”。
...
- 补丁维护：桌面端通过设置页手动录入、结果页当前错题手动扫描或旧版补丁导入纠错；复盘页自动遍历适配器仅属于第一阶段兼容实现
...
- Electron 站点适配器和 2.0.3 核心恢复已完成真实正常模式答题验证。2.1.0 已移除身份识别并加入手动补丁；2.2.0 删除桌面调试模式、重排顶部运行栏并增加右侧答题详情，已完成源码版真实复测。2.3.0 删除向量模型层并优化打包体积，DMG 覆盖安装和真实答题验收均已完成。
...
- Electron 与 Python sidecar 仍是安装包的主要体积来源；发布构建必须确认不含 Torch、Sentence Transformers、M3E 资源，并确认 `prepared` 没有同时进入 `app.asar` 和 `Resources`。
...
- Mac App 的“记录错题”在更多真实结果页变体中是否需要新增站点专用选择器。
